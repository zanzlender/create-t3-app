...
<Quote
  text="I built tRPC to allow people to move faster by removing the need of a traditional API-layer, while still having confidence that our apps won't break as we rapidly iterate."
  author={{
    name: "Alex - creator of tRPC",
    handle: "alexdotjs",
    avatar: "https://avatars.githubusercontent.com/u/459267?v=4",
...
```ts filename="server/api/routers/user.ts"
...
```ts filename="server/api/root.ts"
...
```tsx filename="pages/users/[id].tsx"
...
This video is recommended if you have used tRPC but still feel a bit unclear about
how it works.
...
```ts filename="pages/api/users/[id].ts"
...
```ts filename="pages/api/trpc/[trpc].ts"
import type { NextApiRequest, NextApiResponse } from "next";
